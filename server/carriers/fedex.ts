...
export async function fetchFedExRates(input: CarrierRateInput): Promise<CarrierRate[]> {
  ...
  try {
    ...
    if (data.errors?.length) {
      console.error("[FedEx] Rate API errors:", data.errors.map(e => `${e.code}: ${e.message}`).join("; "));
      return [];
    }
    ...
    for (const detail of replyDetails) {
      ...
      // Prefer ACCOUNT rate detail, fall back to first
    ...
    console.log(`[FedEx] ${rates.length} live REST rates for ${input.destPostal}`);
    return rates;
  } catch (err) {
...
export async function buyFedExLabel(input: CarrierLabelInput): Promise<CarrierLabelResult> {
  ...
  try {
    ...
    if (data.errors?.length) {
      ...
      return { success: false, trackingNumber: "", carrierCode: "fedex", carrierName: "FedEx", service: input.serviceCode ?? "", error: msg };
    }
    ...
    if (!encodedLabel) {
      return { success: false, trackingNumber: "", carrierCode: "fedex", carrierName: "FedEx", service: input.serviceCode ?? "", error: "No label returned from FedEx REST API" };
    }
    ...
    return { success: false, trackingNumber: "", carrierCode: "fedex", carrierName: "FedEx", service: input.serviceCode ?? "", error: `FedEx label error: ${msg}` };
  }
}
