...
const wb = xlsx.readFile("/home/ubuntu/upload/Organika.xlsx");
const ws = wb.Sheets[wb.SheetNames[0]];
const rows = xlsx.utils.sheet_to_json(ws);

console.log(`Found ${rows.length} SKUs in spreadsheet`);

// Organika customer IDs under config 3
const CONFIG_ID = 3;
const CUSTOMER_IDS = [143]; // Organika - B2B
  ...
  for (const customerId of CUSTOMER_IDS) {
    ...
    if (existing.length > 0) {
      await conn.query(
        ...
        [cartonWeightLb, unitsPerCarton, "Imported from Organika.xlsx — full case weight", CONFIG_ID, customerId, sku]
      );
      updated++;
    } else {
      await conn.query(
        "INSERT INTO sku_weight_overrides (config_id, customer_id, sku, carton_weight_lb, units_per_carton, note) VALUES (?, ?, ?, ?, ?, ?)",
        [CONFIG_ID, customerId, sku, cartonWeightLb, unitsPerCarton, "Imported from Organika.xlsx — full case weight"]
      );
      inserted++;
    }
  }
