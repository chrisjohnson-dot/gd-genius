...
function ShipwellStatusBadge({ order, thresholdHours = 2 }: { order: TrackedOrder; thresholdHours?: number }) {
  ...
  // Fetch bids only when panel is open and shipmentId is available
  ...
  const tenderBid = trpc.shipwell.tenderByBidId.useMutation({
    onSuccess: () => {
      setConfirmBid(null);
      setShowBids(false);
      utils.shipwell.listCarrierBids.invalidate({ shipmentId: order.shipwellShipmentId ?? "" });
      utils.pickSchedule.listByChannel.invalidate();
    },
    onError: (err) => {
  ...
  return (
    <div className="inline-block" onClick={(e) => e.stopPropagation()}>
      <div className="inline-flex items-center gap-0.5">
        ...
        {/* View bids toggle — only show when quoting and shipmentId is available */}
