/**
 * Open Orders — D2C
 * Full warehouse-card layout matching the B2B (Home) page.
 * Queries pickSchedule.listByChannel with channel="d2c".
 */
...
> = {
  ...
  qc: {
    ...
    nextStatus: "qc_complete",
    nextLabel: "QC Complete →",
  },
  qc_complete: {
    label: "QC Done",
    bg: "#d1fae5",
    ...
    nextStatus: "ship_ready",
    nextLabel: "Ship Ready →",
  },
  ship_ready: {
    label: "Ship Ready",
    bg: "#dcfce7",
...
function RequiredShipDateBadge({ dateStr }: { dateStr: string }) {
  ...
  if (diffDays < 0) {
    ...
    color = "#b91c1c";
    border = "#fecaca";
    title = `OVERDUE by ${Math.abs(diffDays)}d`;
  } else if (diffDays === 0) {
    bg = "#fff7ed";
    color = "#c2410c";
    border = "#fed7aa";
  ...
  return (
    ...
    >
      ...
      {label}
      {diffDays < 0 && (
        <span className="ml-0.5 font-bold">({Math.abs(diffDays)}d late)</span>
      )}
    </span>
  );
}
...
const SHIPWELL_STATUS_CONFIG: Record<
  string,
...
function ShipwellStatusBadge({
  ...
  const cfg =
    SHIPWELL_STATUS_CONFIG[status] ?? SHIPWELL_STATUS_CONFIG.unknown;
  ...
  const quotingAgeMs = quotingStarted
    ...
    : 0;
  const showZeroBidWarning =
    isQuoting && bidCount === 0 && quotingAgeMs >= thresholdMs;
...
function AdvanceButton({
  ...
  function handleAdvance() {
    if (needsAssociate) setShowDialog(true);
    else
      updateStatus.mutate({
        ...
        >
          ...
          <div className="py-2 space-y-3">
            <p className="text-sm text-muted-foreground">
              ...
              is being given to an associate for picking.
            </p>
            <div className="space-y-1.5">
              <Label
                htmlFor="d2c-associate-name"
                className="text-xs font-semibold"
              >
                Associate Name
              </Label>
              <Input
                id="d2c-associate-name"
                placeholder="Enter associate name…"
                value={associateName}
                onChange={(e) => setAssociateName(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" && associateName.trim())
...
function SlaExtensionButton({
  ...
  const setExt = trpc.sla.setExtension.useMutation({
    onSuccess: () => {
      ...
      onChanged();
      setOpen(false);
      toast.success(`SLA extended by ${days}d`);
    },
    onError: (err) => toast.error(err.message),
  });
  const clearExt = trpc.sla.clearExtension.useMutation({
    onSuccess: () => {
      utils.pickSchedule.listByChannel.invalidate();
      onChanged();
      toast.success("SLA extension cleared");
    },
    onError: (err) => toast.error(err.message),
  });
  return (
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              onClick={(e) => {
                e.stopPropagation();
                setDays(order.slaExtensionDays ?? 1);
                setNote(order.slaExtensionNote ?? "");
                setOpen(true);
              }}
          ...
          <TooltipContent side="left" className="text-xs">
            {hasExtension
              ? `SLA extended +${order.slaExtensionDays}d`
              : "Extend SLA deadline"}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
...
function DismissWarningButton({
  ...
  return (
    <button
      ...
      title="Dismiss zero-bid warning"
...
function WarehouseCard({
  ...
  const [statusFilter, setStatusFilter] = useState<
    LifecycleStatus | "all" | "needs_attention"
  >("all");
  ...
  const clientOptions = useMemo(
    () =>
      Array.from(
        new Map(
          facility.orders.map((o) => [o.clientId, o.clientName])
        ).entries()
      ),
    [facility.orders]
  );
  ...
  const isZeroBidWarning = (o: TrackedOrder) => {
    ...
    const started = o.shipwellQuotingStartedAt
      ? new Date(o.shipwellQuotingStartedAt as string).getTime()
      : 0;
    const thresholdMs =
      resolveThreshold(o.facilityName ?? null) * 60 * 60 * 1000;
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (statusFilter === "needs_attention")
      rows = rows.filter((o) => isZeroBidWarning(o));
    else if (statusFilter !== "all")
      rows = rows.filter((o) => o.lifecycleStatus === statusFilter);
    return [...rows].sort((a, b) => {
      let cmp = 0;
      if (sortKey === "ageDays") cmp = getAgeDays(a) - getAgeDays(b);
      else if (sortKey === "lifecycleStatus")
        cmp = STATUS_RANK[a.lifecycleStatus] - STATUS_RANK[b.lifecycleStatus];
      else if (sortKey === "totalPieces")
        cmp = (a.totalPieces ?? 0) - (b.totalPieces ?? 0);
      else
        cmp = String(a[sortKey as keyof TrackedOrder] ?? "").localeCompare(
          String(b[sortKey as keyof TrackedOrder] ?? "")
        );
  ...
  const groupedByClient = useMemo(() => {
    const map = new Map<
      number,
      { clientId: number; clientName: string; orders: TrackedOrder[] }
    >();
    ...
    for (const g of groups) {
      g.orders.sort((a, b) => {
        ...
        return (
          new Date(a.requiredShipDate).getTime() -
          new Date(b.requiredShipDate).getTime()
        );
      });
    }
    return groups;
  ...
  const counts = useMemo(() => {
    const c: Record<LifecycleStatus, number> = {
      unallocated: 0,
      allocated: 0,
      picking: 0,
      qc: 0,
  ...
  const exportCsv = () => {
    ...
    a.download = `${facility.facilityName.replace(/[^a-z0-9]/gi, "_")}_d2c_orders.csv`;
    a.click();
  ...
  const exportPdf = () => {
    ...
    doc.setFontSize(14);
    doc.text(
      `${facility.facilityName} — D2C Open Orders`,
      40,
      36
    );
    doc.setFontSize(9);
    doc.text(
      `Exported ${new Date().toLocaleString()} · ${filteredOrders.length} orders`,
      40,
      52
    );
    autoTable(doc, {
    ...
    doc.save(
      `${facility.facilityName.replace(/[^a-z0-9]/gi, "_")}_d2c_orders.pdf`
    );
  ...
  const tableHeader = (showClient: boolean) => (
    <thead>
      <tr>
        ...
        >
          PO #{" "}
          <SortIcon col="poNum" sortKey={sortKey} sortDir={sortDir} />
        </th>
        {showClient && (
          <th
          ...
          >
            Client{" "}
            <SortIcon col="clientName" sortKey={sortKey} sortDir={sortDir} />
          </th>
        )}
        <th
          onClick={() => toggleSort("shipToName")}
          className="cursor-pointer select-none"
        >
          Ship To{" "}
          <SortIcon col="shipToName" sortKey={sortKey} sortDir={sortDir} />
        </th>
        <th
          onClick={() => toggleSort("shipToCity")}
          className="cursor-pointer select-none"
        >
          City{" "}
          <SortIcon col="shipToCity" sortKey={sortKey} sortDir={sortDir} />
        </th>
        <th
          onClick={() => toggleSort("requiredShipDate")}
          className="cursor-pointer select-none"
        >
          Req. Ship{" "}
          <SortIcon
            col="requiredShipDate"
            sortKey={sortKey}
            sortDir={sortDir}
          />
        </th>
        ...
        >
          Age{" "}
          <SortIcon col="ageDays" sortKey={sortKey} sortDir={sortDir} />
        </th>
        <th
          onClick={() => toggleSort("totalPieces")}
          className="cursor-pointer select-none text-right"
        >
          Pcs{" "}
          <SortIcon col="totalPieces" sortKey={sortKey} sortDir={sortDir} />
        </th>
  ...
  const orderRow = (o: TrackedOrder, showClient: boolean) => {
    ...
    return (
      ...
      >
        ...
        <td className="text-muted-foreground text-xs">{o.shipToCity ?? "—"}</td>
        ...
        <td className="text-right">
          {(() => {
            if (!o.requiredShipDate)
              return (
                <span className="text-muted-foreground text-xs">—</span>
              );
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            const target = new Date(o.requiredShipDate);
            target.setHours(0, 0, 0, 0);
        ...
        <td className="text-muted-foreground text-xs text-right">
          {(o.totalPieces ?? 0) > 0
            ? (o.totalPieces ?? 0).toLocaleString()
            : "—"}
        </td>
        <td className="text-muted-foreground text-xs text-right">
          ...
          <div className="flex items-center justify-end gap-1">
            {o.shipwellShipmentId ? (
              <>
                <ShipwellStatusBadge
                  order={o}
                  thresholdHours={resolveThreshold(o.facilityName ?? null)}
                />
  ...
  const filterBar = (
    <div className="px-5 py-2.5 border-b border-border flex flex-wrap items-center gap-2 bg-muted/10 shrink-0">
      ...
      <select
        value={statusFilter}
        onChange={(e) =>
          setStatusFilter(e.target.value as LifecycleStatus | "all")
        }
        className="py-1.5 px-2.5 text-xs rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-primary/30"
      >
        <option value="all">All Statuses</option>
        {(
          Object.entries(LIFECYCLE_CONFIG) as [
            LifecycleStatus,
            (typeof LIFECYCLE_CONFIG)[LifecycleStatus]
          ][]
        ).map(([s, c]) => (
      ...
      {(search || clientFilter !== "all" || statusFilter !== "all") && (
        <button
          ...
          className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground px-2 py-1.5 rounded-lg hover:bg-muted/50 transition-colors"
        >
          <X className="h-3 w-3" />
          Clear
        </button>
      ...
      <table className="w-full data-table">
        ...
        <tbody>
          {groupedByClient.map((group) => {
            const groupPieces = group.orders.reduce(
              (s, o) => s + (o.totalPieces ?? 0),
              0
            );
            const unallocCount = group.orders.filter(
              (o) => o.lifecycleStatus === "unallocated"
            ).length;
            return [
              <tr
              ...
              >
                ...
                <td className="py-2 px-3 text-[10px] text-gray-400 font-medium">
                  PO #
                </td>
                <td className="py-2 px-3 text-[10px] text-gray-400 font-medium">
                  Ship To
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                {facility.orders.length} order
                {facility.orders.length !== 1 ? "s" : ""} ·{" "}
                {clientOptions.length} client
                {clientOptions.length !== 1 ? "s" : ""}
                {overdueCount > 0 && (
                  <span
                    className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold"
                    style={{
            ...
            <div className="hidden md:flex items-center gap-2">
              {(
                ...
                >
                  ...
                  <p
                    className="text-[8px] mt-0.5 font-medium uppercase tracking-wide"
                    style={{ color: cfg.text }}
                  >
                    {cfg.label}
                  </p>
  ...
  return (
    ...
    >
      <div
        className={`px-5 py-4 select-none bg-card border-b border-border ${
          onDrillDown
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                ...
                {clientOptions.length !== 1 ? "s" : ""}
                {(() => {
                  const unalloc = facility.orders.filter(
                    (o) => o.lifecycleStatus === "unallocated"
                  ).length;
                  return unalloc > 0 ? (
                    <span
        ...
        <div className="flex flex-wrap gap-1.5">
          {(
            ...
            >
              ...
              <p
                className="text-[9px] mt-0.5 font-medium uppercase tracking-[0.04em] truncate"
                style={{ color: cfg.text }}
              >
                {cfg.label}
              </p>
          ...
          {needsAttentionCount > 0 && (
            <div
              ...
              style={{
                background: "#fff7ed",
                borderColor:
                  statusFilter === "needs_attention" ? "#ea580c" : "#fed7aa",
                opacity:
              ...
              onClick={(e) => {
                e.stopPropagation();
                setStatusFilter((f) =>
                  f === "needs_attention" ? "all" : "needs_attention"
                );
                if (!expanded) setExpanded(true);
              }}
            ...
            >
              ...
              <p className="text-[9px] mt-1 font-medium uppercase tracking-wide text-orange-600">
                Attention
              </p>
            </div>
          )}
...
function SlaBreachSummarySection({
  ...
  const totalBreached = breachGroups.reduce(
    (s, g) => s + g.breachCount,
    0
  );
  ...
  const warehouses = Array.from(warehouseMap.values())
    ...
    .map((wh) => ({
      ...wh,
      clients: [...wh.clients].sort(
        (a, b) => b.worstDaysOverdue - a.worstDaysOverdue
      ),
    }));

  return (
    <div
      className="bg-card border-2 border-red-200 rounded-2xl overflow-hidden"
      style={{
        boxShadow:
          "0 0 0 1px rgba(239,68,68,0.1), 0 4px 16px rgba(239,68,68,0.08)",
      }}
    >
              ...
              >
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 border border-red-200">
                    {wh.totalBreached} order
                    {wh.totalBreached !== 1 ? "s" : ""}
                  </span>
                  <span className="text-[10px] text-muted-foreground">
                    {wh.clients.length} client
                    {wh.clients.length !== 1 ? "s" : ""}
                  </span>
                </div>
                <span className="text-red-600 font-bold text-xs">
                  {wh.worstDaysOverdue}d worst overdue
                </span>
              </button>
              {!isCollapsed && (
                <table className="w-full data-table">
                    ...
                    {wh.clients.map((group) => {
                      ...
                      return (
                        <tr key={group.clientId}>
                          ...
                          <td className="text-right">
                            <span className="text-red-600 font-bold text-xs">
                              {group.worstDaysOverdue}d overdue
                            </span>
                          </td>
                          <td className="text-muted-foreground font-mono text-xs">
                            {worst?.referenceNum ??
                              (worst
                                ? `#${worst.extensivOrderId}`
                                : "—")}
...
export default function OpenOrdersD2C() {
  ...
  const lastSyncAt = data?.lastSyncAt
    ? new Date(data.lastSyncAt as string | Date)
    : null;
  ...
  const { data: verifData } =
    trpc.allocation.unresolvedVerificationCount.useQuery();
  ...
  const facilityOverdueCounts = useMemo(() => {
    ...
    for (const g of breachData ?? []) {
      if (g.facilityId != null)
        map.set(
          g.facilityId,
          (map.get(g.facilityId) ?? 0) + g.breachCount
        );
    }
    return map;
  }, [breachData]);
    ...
    <div className="p-5 space-y-4 page-enter">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="page-breadcrumb">
              {selectedFacility
                ? `Open Orders › ${selectedFacility.facilityName}`
                : "Overview"}
            </p>
          ...
          >
            <RefreshCw
              className={`h-3.5 w-3.5 ${
                syncNow.isPending || syncRunning ? "animate-spin" : ""
              }`}
            />
            Sync Now
          </Button>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
            {(
              ...
              >
                ...
                <p
                  className="text-[9px] mt-1 font-semibold uppercase tracking-[0.04em] truncate"
                  style={{ color: cfg.text }}
                >
                  {cfg.label}
                </p>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p
                    ...
                    style={{ color: "#991b1b" }}
                  >
                    {verifData!.count} Allocation Run
                    {verifData!.count !== 1 ? "s" : ""} with Verification
                    Issues
                  </p>
                  <p className="text-xs mt-0.5" style={{ color: "#b91c1c" }}>
                    Extensiv reported a mismatch or partial allocation — review
                    in Run History
                  </p>
                </div>
                <span
      ...
      {!isLoading && orders.length === 0 && (
        <div className="text-center py-20 text-muted-foreground">
          ...
          <p className="text-xs mt-1 opacity-70">
            Click <strong>Sync Now</strong> to pull open orders from Extensiv,
            or assign D2C clients in{" "}
            <Link
              href="/client-visibility"
      ...
      {!isLoading && selectedFacility && (
        <WarehouseCard
          ...
          drillDown
          laneThresholds={
            (data?.laneThresholds ?? []) as LaneThresholdEntry[]
          }
          initialClientFilter={pendingClientFilter}
          overdueCount={
            facilityOverdueCounts.get(selectedFacility.facilityId) ?? 0
          }
        />
      )}
