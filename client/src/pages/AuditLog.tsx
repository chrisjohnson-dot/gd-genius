...
export default function AuditLog() {
  ...
  const hasFilter = actionFilter !== "all";
      ...
      <div className="p-6 space-y-6 max-w-5xl">
        ...
        {!isLoading && logs && (
          <p className="text-xs text-muted-foreground -mt-3">
            {hasFilter
              ? `Showing ${logs.length} entr${logs.length !== 1 ? "ies" : "y"} for "${formatAction(actionFilter)}"`
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-muted-foreground">
              {hasFilter ? `Filtered: ${formatAction(actionFilter)}` : "Recent Actions"}
            </CardTitle>
          </CardHeader>
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">
                  {hasFilter ? `No entries found for "${formatAction(actionFilter)}".` : "No audit entries yet."}
                </p>
                {hasFilter && (
                  <button
