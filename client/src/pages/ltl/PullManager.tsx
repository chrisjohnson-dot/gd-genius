...
/** Convert a local date string (YYYY-MM-DD) to start-of-day or end-of-day UTC ms */
...
function ExportDialog({
  ...
  const handleExport = async () => {
    setIsExporting(true);
    try {
      const { csv } = await utils.pullTracker.exportSessions.fetch({
      ...
      const whLabel = warehouse === "all" ? "ALL" : warehouse;
      downloadCsv(csv, `pull-sessions_${whLabel}_${fromLabel}-${toLabel}.csv`);
      onClose();
    } catch (e: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Export Pull Sessions</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Exports up to 5,000 sessions as a CSV file with Session ID, Pick Ticket, Associate, Warehouse, Status, Timestamps, Duration, Pallets, Cases, Items/hr, OpFi status, and Notes.
          </p>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose} disabled={isExporting}>Cancel</Button>
          <Button onClick={handleExport} disabled={isExporting} className="bg-[#15527f] hover:bg-[#1a6699] text-white gap-2">
...
function TrendIcon({
  ...
  recentIph: number | null;
  priorIph: number | null;
}) {
  const tooltipText =
    direction === "new"
      ? "New associate — no prior 7-day data"
      : direction === "flat"
      ? `Flat — last 7d: ${recentIph?.toFixed(1)} vs prior 7d: ${priorIph?.toFixed(1)} items/hr`
      : `${direction === "up" ? "↑ Improving" : "↓ Declining"} — last 7d: ${recentIph?.toFixed(1)} vs prior 7d: ${priorIph?.toFixed(1)} items/hr`;

  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className="inline-flex items-center">
          {direction === "up" && <TrendingUp className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />}
          {direction === "down" && <TrendingDown className="h-4 w-4 text-red-600 dark:text-red-400" />}
          {direction === "flat" && <Minus className="h-4 w-4 text-amber-500" />}
          {direction === "new" && <span className="text-[10px] font-semibold text-muted-foreground px-1 py-0.5 bg-muted rounded">NEW</span>}
        </span>
      </TooltipTrigger>
      <TooltipContent className="text-xs max-w-48">{tooltipText}</TooltipContent>
    </Tooltip>
  );
}

// ─── Main Page ────────────────────────────────────────────────────────────────
// Default labor cost per hour for Cost/Case calculation (can be made configurable later)
const DEFAULT_LABOR_COST_PER_HOUR = 18;
...
export default function PullManager() {
  ...
  const today = msToDateStr(Date.now());
  const sevenDaysAgo = msToDateStr(Date.now() - 7 * 24 * 3600 * 1000);
  ...
  const { data: sessions = [], isLoading, refetch } = trpc.pullTracker.listSessions.useQuery({
    ...
    limit: 100,
    dateFrom: dateFilterActive && dateFrom ? dateStrToMs(dateFrom, false) : undefined,
    dateTo: dateFilterActive && dateTo ? dateStrToMs(dateTo, true) : undefined,
  ...
  const totalPallets = completedSessions.reduce((sum, s) => sum + s.totalPallets, 0);
  const totalCases = completedSessions.reduce((sum, s) => sum + s.totalCases, 0);
  const avgDuration = completedSessions.filter(s => s.durationSeconds).length > 0
    ? Math.round(completedSessions.filter(s => s.durationSeconds).reduce((sum, s) => sum + (s.durationSeconds ?? 0), 0) / completedSessions.filter(s => s.durationSeconds).length)
    : 0;

  // Unique warehouses from session list for export dialog
  const warehouseOptions = useMemo(
    () => Array.from(new Set(sessions.map(s => s.warehouseId))).sort(),
    [sessions]
  );

  // Compute Cost/Case for each session: (durationSeconds / 3600) * laborCostPerHour / totalCases
  function costPerCase(durationSeconds: number | null | undefined, totalCases: number): string {
    if (!durationSeconds || totalCases === 0) return "—";
    const cost = (durationSeconds / 3600) * DEFAULT_LABOR_COST_PER_HOUR / totalCases;
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                ...
                Associate Efficiency
                <span className="ml-auto text-xs font-normal text-muted-foreground">7-day trend vs prior 7 days</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Table>
                  ...
                  {(stats as any[]).map((s) => (
                    <TableRow key={s.associateId}>
                      ...
                      <TableCell className="text-center">
                        <TrendIcon
                          direction={s.trendDirection ?? "new"}
                          recentIph={s.trendRecentIph ?? null}
                          priorIph={s.trendPriorIph ?? null}
                        />
                      </TableCell>
                    ...
                    <TableHead className="text-right">
                      <Tooltip>
                        ...
                        <TooltipContent className="text-xs max-w-52">
                          Estimated labor cost per case based on ${DEFAULT_LABOR_COST_PER_HOUR}/hr. Configure in Alert Settings.
                        </TooltipContent>
                      </Tooltip>
                    </TableHead>
                ...
                <TableBody>
                  {sessions.map((s) => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell className="text-right">
                        <span className={`text-sm font-semibold ${s.status === "completed" && s.totalCases > 0 ? "text-foreground" : "text-muted-foreground"}`}>
                          {s.status === "completed" ? costPerCase(s.durationSeconds, s.totalCases) : "—"}
                        </span>
                      </TableCell>
