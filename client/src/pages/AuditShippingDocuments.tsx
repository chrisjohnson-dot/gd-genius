...
export default function AuditShippingDocuments() {
  ...
  async function handleGenerate() {
    ...
    try {
      const resp = await fetch("/api/pdf/audit-shipping-documents", {
        method: "POST",
      ...
      a.href = url;
      a.download = `audit-shipping-documents-${Date.now()}.pdf`;
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Reproduce Bills of Lading from Extensiv with an{" "}
            <span className="font-semibold text-red-600">AUDIT</span> watermark
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Fetches each order from Extensiv and renders a BOL-style PDF with an AUDIT watermark on every page.
            Includes carrier, tracking, ship-from/to addresses, and line items.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button
          ...
          >
            {generating ? (
              ...
              <>
                ...
                Generate Audit Shipping Documents PDF
              </>
            )}
          </Button>
          ...
          {downloadUrl && !genError && (
            <div className="flex items-center gap-3 rounded-lg border border-green-300/40 bg-green-50 dark:bg-green-950/20 px-4 py-3">
              ...
              <a
                ref={downloadLinkRef}
                href={downloadUrl}
                download={`audit-shipping-documents-${Date.now()}.pdf`}
                className="shrink-0"
              >
                <Button
                  variant="outline"
